...
class Regression:
    def __init__(self, csv_file = None, data = None, values = None):
        ...
        def __validation_data(self, holdout):
            test_rows = random.sample(self.df.index, int(roud(len(self.df) * holdout)))
            ...
            train_valuse = self.values.ix[train_rows]
            kd = Regression(data = df_train, values = train_valuse)
